 ...
 *
 * IN THIS LAB the control-boundary half is the live one, and the whole palette
 * was designed around it. Every panel here sits on a surface only 1.14:1
 * against the page, so the fill can never carry the boundary and the entire
 * burden falls on the border - which is why `--line` (#6e6e8f) is as bright as
 * it is: it clears 3.3:1 against every surface in `style.css`, and that is a
 * measured claim rather than a design intention, which is exactly what this
 * oracle checks.
 *
 * Four shapes on this page are worth naming because they are where a boundary
 * usually goes missing: `#app button.primary` fills with `--accent` and takes
 * a DELIBERATELY DIFFERENT border (`--accent-bright`), because a border
 * painted the same colour as its own fill leaves a control with no edge of its
 * own; `#app button.danger` does the same with `--alarm`; the segmented
 * ceremony-mode buttons keep their own `--line-strong` border in BOTH the
 * pressed and unpressed states rather than leaning on a wrapper; and
 * `input[aria-invalid="true"]` recolours to `--alarm` at 2px, a state only the
 * drive reaches. This oracle judges each control as painted, at every driven
 * state.
 *
 * The generated-content half is inert in this repo today - the stylesheet
 * declares no `content` at all, and the verdict glyphs are real text nodes
 * rather than pseudo-elements precisely so the contrast walk can reach them.
 * The only generated marks are the `<summary>` disclosure triangles, which are
 * `::marker`, the UA's own. It runs anyway, at every state, so that stays a
...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     *
     * A driven pass here walks one long document with every act rendered at
     * once, and the expensive part is the tables: eighteen ceremony audit rows,
     * ten comparison columns, eight tableau rows and seven agreement rows -
