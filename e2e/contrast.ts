 ...
 *
 *  - TEXT OVER A SURFACE AXE DECLINES TO RESOLVE. The shared top bar's ink is
 *    `color-mix(in srgb, var(--accent) 60%, #eafff8)` and its border and hover
 *    fill are `color-mix()` over `transparent` - so every control in the bar,
 *    on every lab in this fleet, lands in axe's `incomplete` bucket rather
 *    than being judged. This lab's own palette is deliberately plain hex on
 *    plain hex for exactly that reason, but the walk resolves composites
 *    anyway, because the bar is not this lab's to simplify and a future token
 *    here could reintroduce the shape.
 *
 *  - TEXT FADED BY AN ANCESTOR'S `opacity` - axe reads the declared `color`,
 *    which is not the colour that lands on screen. `style.css` avoids that
 *    everywhere it dims text, lowering the colour's LIGHTNESS instead
 *    (`--text-dim`, `--text-muted`), and the walk is what makes that a
 *    measurement rather than a convention. The one `opacity` left in the
 *    stylesheet is none at all; `#app button:disabled` recolours instead, and
 *    disabled controls are exempt as inactive components and skipped below.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. This is the live
 *    case here rather than a precaution: this lab renders eight wide tables,
 *    every one of them inside a `.scroll-x` region with `overflow-x: auto`, and
 *    at 380px most of their columns really are scrolled out of view. Content
 ...
 *    outside every ancestor's box, so the walk would find nothing behind it
 *    and fall through to WHITE, inventing a failure on text that is simply
 *    off to the right of a scrolled table.
 *
 ...
 * and banner glyphs - the `[+]`, `[x]`, `[!]` and `[ ]` marks in
 * `.verdict-icon` - each painted in the same semantic ink as the word beside
 * it (`--ok`, `--warn`, `--alarm`, `--text-dim`). Those are the states this lab
 * exists to show, so `scan()` calls this a second time as
...
export async function auditContrast(
  ...
  return page.evaluate(([rootSelector, allowAriaHidden]: [string, boolean]) => {
     ...
     *
     * A driven pass here walks a single long document with every act rendered
     * at once, and the expensive part is the tables: the ceremony audit alone
     * is eighteen rows of three cells, the comparison table is ten columns, and
     * the division tableau is eight rows of six - all of them siblings
     * re-walking the same ancestors up to `<body>`, on top of several hundred
     * characters of hex per commitment. Without the caches the pass re-reads
     ...
     * that is every `.scroll-x` table wrapper, of which this page has eight,
     * plus whatever the shared top bar clips. The collection is built from the
     * live DOM, so a new scroller joins it without an edit here.
     */
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // of the shape: all eleven `details.deep` progressive-disclosure panels
      // ship shut, and they hold most of the page's depth. The gate opens them
      // by clicking their <summary>, which is the route a reader has, rather
      // than setting `.open` from script - which is what the retired gate did,
      // to every <details> on the page, before its only scan.
     ...
     *
     * Every `aria-hidden` element on this page is a `.verdict-icon` span - the
     * `[+]` / `[x]` / `[!]` / `[ ]` glyph on each verdict box and each banner -
     * plus the shared header's two SVG marks, which carry no text. Each glyph
     * duplicates the word directly beside it, but each is painted in a SEMANTIC
     * ink (`--ok`, `--warn`, `--alarm`, `--text-dim`), which is why `scan()`
     ...
     *
     * Nothing on this page hides a VALUE. No commitment, proof, recovered
     * trapdoor, failure code or verdict text is inside an `aria-hidden`
     * subtree - which was checked rather than assumed, because that is the
     * shared blind spot where both oracles stop looking and it is the one
     * place a live readout can hide from a whole accessibility gate.
     */
